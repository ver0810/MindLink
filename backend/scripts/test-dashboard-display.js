...
async function testDashboardDisplay() {
  ...
  try {
    ...
    const mockConversation = {
      id: 1,
      title: "学习方法咨询",
      primary_mentor_name: "李教授",
      mentor_name: "李教授",
      messageCount: 8,
      message_count: 8,
      last_message: "感谢您的建议，我会认真运用这些学习方法和记忆技巧。",
      ...
      // 分析相关字段
      summary:
        "用户咨询如何提高学习效率和记忆力，讨论了学习策略、记忆技巧、时间管理等关键要素，并获得了针对性的建议和实施方案。",
      aiSummary:
        "用户咨询如何提高学习效率和记忆力，讨论了学习策略、记忆技巧、时间管理等关键要素，并获得了针对性的建议和实施方案。",
      ...
      keyTopics: ["学习方法", "记忆技巧", "时间管理", "学习计划"],
      key_topics: ["学习方法", "记忆技巧", "时间管理", "学习计划"],

      autoTags: ["学习咨询", "记忆研究", "效率提升"],
      auto_tags: ["学习咨询", "记忆研究", "效率提升"],
    ...
    const problemCategories =
      mockConversation.problemCategories ||
      mockConversation.problem_categories ||
      [];
    const keyTopics =
      mockConversation.keyTopics || mockConversation.key_topics || [];
    const autoTags =
      mockConversation.autoTags || mockConversation.auto_tags || [];
    const complexityLevel =
      mockConversation.complexityLevel ||
      mockConversation.complexity_level ||
      1;

    console.log("✓ 字段提取测试通过");
    console.log(
      `  - AI总结: ${summary ? "✓" : "✗"} (${summary ? summary.length : 0}字符)`
    );
    console.log(
      `  - 问题类型: ${problemCategories.length}个 [${problemCategories
        .slice(0, 3)
        .join(", ")}]`
    );
    console.log(
      `  - 关键话题: ${keyTopics.length}个 [${keyTopics
        .slice(0, 2)
        .join(", ")}]`
    );
    console.log(
      `  - 智能标签: ${autoTags.length}个 [${autoTags.slice(0, 2).join(", ")}]`
    );
    ...
    console.log(
      `  - 复杂度等级: ${complexityLevel} (${getComplexityLabel(
        complexityLevel
      )})`
    );
    console.log("  - 指示器颜色:");
    ...
    const summaryHtml = summary
      ? `
            <div class="bg-blue-900/30 border border-blue-500/30 rounded-md p-3 mb-3">
                ...
                <p class="text-blue-200 text-sm leading-relaxed line-clamp-2">${escapeHtml(
                  summary
                )}</p>
            </div>
        `
            ...
            <div class="flex items-center space-x-2 mb-3">
                ...
                <div class="flex space-x-1">
                    ${Array.from({ length: 5 }, (_, i) => {
                      ...
                      return `<div class="w-2 h-2 rounded-full ${
                        isActive ? "opacity-100" : "opacity-30"
                      }"
                                     style="background-color: ${color}"></div>`;
                    }).join("")}
                </div>
                <span class="text-xs text-slate-400">${getComplexityLabel(
                  complexityLevel
                )}</span>
            </div>
        `
    ...
    const shouldShowAnalyzeButton =
      !summary && mockConversation.messageCount >= 3;
    console.log(`✓ 一键分析按钮逻辑测试通过`);
    console.log(`  - 显示条件: 无总结 && 消息数≥3`);
    console.log(
      `  - 当前状态: 无总结=${!summary}, 消息数=${
        mockConversation.messageCount
      }`
    );
