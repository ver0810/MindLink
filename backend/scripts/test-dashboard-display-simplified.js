...
const mockConversations = [
  {
    ...
    key_topics: ["记忆", "复习", "方法"], // 应该不会显示
    auto_tags: ["study", "learning"], // 应该不会显示
  ...
  },
  {
    ...
    key_topics: ["考试", "复习"], // 应该不会显示
    auto_tags: ["exam"], // 应该不会显示
...
function testSimplifiedDisplay(conversation) {
  ...
  console.log(
    `  - 问题类型: [${problemCategories.map(formatTagName).join(", ")}]`
  );
  ...
  console.log(
    `  ✓ AI总结显示: ${shouldShowSummary ? "显示" : "不显示"} ${
      !shouldShowSummary ? "✅" : "❌"
    }`
  );
