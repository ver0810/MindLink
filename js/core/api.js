...
const APIService = {
    ...
    buildSystemPrompt(mentorsInfo, mode) {
        if (mode === '1v1') {
            ...
            return `你是${mentor.name}，${mentor.title}。你的专业领域包括：${mentor.expertise.join('、')}。

个人简介：${mentor.bio}

请严格按照${mentor.name}的身份、专业背景和思维方式来回答问题。保持角色的一致性，提供专业、实用的建议。回答要有深度，结合实际案例，体现出${mentor.name}独特的见解和经验。
...
4. 回答要有深度和价值`;
            ...
            const allExpertise = [...new Set(mentorsInfo.flatMap(m => m.expertise))];
