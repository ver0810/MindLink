document.addEventListener("DOMContentLoaded", function () {
  ...
  function updateNavState(activeSection) {
    ...
    const mobileConversationsLink = document.getElementById(
      "mobile-conversations-link"
    );
    ...
    if (activeSection === "mentors") {
      ...
      const activeConversationLinks = [
        conversationsLink,
        mobileConversationsLink,
      ];
    ...
    if (typeof mentors === "undefined") {
      ...
      if (allMentorsContainer) {
        allMentorsContainer.innerHTML =
          '<p class="text-red-400 text-center col-span-full">导师数据加载失败，请稍后再试。</p>';
      }
      return;
    }
  ...
  function renderMentorCard(mentor, containerId) {
    ...
    mentorCard.className =
      "cool-card cool-card-hover p-6 flex flex-col items-center text-center cursor-pointer h-full"; // Added h-full for consistent card height if using grid
    ...
    mentorCard.innerHTML = `
      ...
      mentor.name
            ...
            <p class="text-slate-400 text-xs mb-4 line-clamp-3 flex-grow">${
              mentor.shortBio
            }</p>
            <div class="flex flex-wrap gap-2 justify-center">
                ...
                ${
                  mentor.expertise.length > 3
                    ? `<span class="inline-block text-slate-500 text-xs px-2.5 py-1">+${
                        mentor.expertise.length - 3
                      }更多</span>`
                    : ""
                }
  ...
  function populateMentors(filterTerm = "") {
    const featuredMentorsContainer =
      document.getElementById("featured-mentors");
    const allMentorsContainer = document.getElementById("all-mentors");
    const featuredMentorsSection = document.getElementById(
    ...
    if (searchTerm) {
      displayedMentors = mentors.filter(
        (mentor) =>
      ...
      if (featuredMentorsSection)
        featuredMentorsSection.classList.add("hidden"); // Hide featured section during search
    } else {
      if (featuredMentorsSection)
        featuredMentorsSection.classList.remove("hidden");
      ...
      if (featuredMentorsContainer) {
        ...
        console.log("Featured mentors count:", featuredMentorsList.length);
        console.log(
          "Featured mentors:",
          featuredMentorsList.map((m) => m.name)
        );
        featuredMentorsList.forEach((mentor) =>
          renderMentorCard(mentor, "featured-mentors")
        );
      }
    }
    ...
    if (allMentorsContainer) {
      if (displayedMentors.length === 0 && searchTerm) {
        ...
        console.log("All mentors to display:", displayedMentors.length);
        displayedMentors.forEach((mentor) =>
          renderMentorCard(mentor, "all-mentors")
        );
      }
    }
  }
  ...
  function initMentorModal() {
    ...
    window.addEventListener("keydown", (event) => {
      if (
        event.key === "Escape" &&
        mentorModal &&
        mentorModal.classList.contains("flex")
      ) {
        closeMentorModal();
      }
    });
    ...
    window.openMentorModal = function (mentor) {
      ...
      try {
        ...
        const expertiseContainer = document.getElementById(
          "modal-mentor-expertise"
        );
        ...
        const questionsContainer = document.getElementById(
          "modal-mentor-questions"
        );
        if (questionsContainer) {
          questionsContainer.innerHTML = mentor.suggestedQuestions
            .map(
              (question) =>
...
class DashboardConversationHistory {
  ...
  setupModalEvents() {
    ...
    if (toggleFavorite) {
      toggleFavorite.addEventListener("click", () => {
        const conversationId = toggleFavorite.getAttribute(
          "data-conversation-id"
        );
        const isFavorite =
          toggleFavorite.getAttribute("data-is-favorite") === "true";
  ...
  async loadStatistics() {
    try {
      const response = await fetch(
        "http://localhost:3000/api/conversations/stats/overview",
        {
          headers: {
  ...
  renderStatistics() {
    ...
    if (favoriteConv)
      favoriteConv.textContent = basicStats.favorite_conversations || 0;
    if (archivedConv)
      archivedConv.textContent = basicStats.archived_conversations || 0;
  }
  ...
  async loadConversations(reset = true) {
    ...
    try {
      ...
      const response = await fetch(
        `http://localhost:3000/api/conversations?${params}`,
        {
          headers: {
            Authorization: `Bearer ${localStorage.getItem("auth_token")}`,
          },
  ...
  createConversationCard(conversation) {
    ...
    card.className =
      "bg-slate-800 rounded-lg p-6 border border-slate-700 hover:border-slate-600 transition-colors cursor-pointer";

    const mentorName =
      conversation.primary_mentor_name ||
    ...
    const problemCategories =
      conversation.problemCategories || conversation.problem_categories || [];
    ...
    const complexityLevel =
      conversation.complexityLevel || conversation.complexity_level || 1;
            ...
            <div class="flex justify-between items-start mb-4">
                <div class="flex-1">
                    <h3 class="font-semibold text-lg text-slate-100 mb-2 line-clamp-1">
                        ${this.escapeHtml(
                          conversation.title || `与${mentorName}的对话`
                        )}
                    </h3>
  ...
  showConversationModal(conversation) {
    ...
    title.textContent =
      conversation.title ||
      `与${conversation.primary_mentor_name || conversation.mentor_name}的对话`;
  ...
  async toggleFavorite(conversationId, isFavorite) {
    try {
      const response = await fetch(
        `http://localhost:3000/api/conversations/${conversationId}/favorite`,
        {
          method: "POST",
          headers: {
      ...
      confirmModal.innerHTML = `
                <div class="bg-slate-800 rounded-lg max-w-md w-full p-6 border border-slate-700">
                    ...
                    <div class="mb-6">
                        ...
                        <p class="text-white font-medium bg-slate-700 p-3 rounded border-l-4 border-red-500">"${this.escapeHtml(
  ...
  exportConversation(conversation) {
    const content = [
      `# ${
        conversation.title ||
        `与${
          conversation.primary_mentor_name || conversation.mentor_name
        }的对话`
      }`,
      "",
      `**导师**: ${
      ...
      `**创建时间**: ${new Date(conversation.created_at).toLocaleString(
        "zh-CN"
      )}`,
      `**更新时间**: ${new Date(conversation.updated_at).toLocaleString(
        "zh-CN"
      )}`,
    ...
    a.href = url;
    a.download = `${conversation.title || "对话记录"}_${
  ...
  async loadMentorFilter() {
    ...
    try {
      const response = await fetch(
        "http://localhost:3000/api/conversations/mentors",
        {
          headers: {
            Authorization: `Bearer ${localStorage.getItem("auth_token")}`,
          },
  ...
  formatTagName(tagName) {
    ...
    return (
      tagMappings[tagName] || formatted.replace(/\b\w/g, (l) => l.toUpperCase())
    );
  }
